...
(function (pxt) {
    ...
    (function (Cloud) {
        ...
        function parseScriptId(uri) {
            ...
            var m = new RegExp("^((https://)?" + embedUrl + ")?(api/oembed?url=.*%2F([^&]*)&.*?|([a-z0-9-]+))$", 'i').exec(uri.trim());
            var scriptid = m && (!m[1] || m[1] == "https://" + Util.stripUrlProtocol(target.appTheme.embedUrl)) && (m[3] || m[4]) ? (m[3] ? m[3].toLowerCase() : m[4].toLowerCase()) : null;
            return scriptid;
        }
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        (function (decompiler) {
            ...
            function checkStatement(node, blocksInfo) {
                switch (node.kind) {
                    case SK.WhileStatement:
                    case SK.IfStatement:
                    case SK.Block:
                        return undefined;
                    case SK.ExpressionStatement:
                        return checkStatement(node.expression, blocksInfo);
                    case SK.VariableStatement:
                        return checkVariableStatement(node, blocksInfo);
                    case SK.CallExpression:
                        return checkCall(node, blocksInfo);
                    case SK.VariableDeclaration:
                    ...
                    case SK.BinaryExpression:
                        return checkBinaryExpression(node, blocksInfo);
                    case SK.ForStatement:
                ...
                function checkBinaryExpression(n, blocksInfo) {
                    if (n.left.kind !== SK.Identifier) {
                ...
                function checkVariableDeclaration(n, blocksInfo) {
                    var check;
                ...
                function checkVariableStatement(n, blocksInfo) {
                    for (var _i = 0, _a = n.declarationList.declarations; _i < _a.length; _i++) {
                        var declaration = _a[_i];
                        var res = checkVariableDeclaration(declaration, blocksInfo);
                        if (res) {
                            return res;
                        }
                    }
                ...
                function checkCall(n, blocksInfo) {
                    var info = n.callInfo;
                    if (!info) {
                    ...
                    if (api) {
                        var ns = blocksInfo.apis.byQName[api.namespace];
            ...
            function checkExpression(n, blocksInfo) {
                switch (n.kind) {
                    case SK.StringLiteral:
                    case SK.FirstTemplateToken:
                    case SK.NoSubstitutionTemplateLiteral:
                    ...
                    case SK.ExpressionStatement:
                        return undefined;
                    case SK.ParenthesizedExpression:
                        return checkExpression(n.expression, blocksInfo);
                    case SK.Identifier:
            ...
            if (node0.kind == pxtc.SK.FunctionDeclaration && res.block === "true" && !res.blockId) {
                ...
                if (fn.symbol.parent) {
                    ...
                    res.block = "" + node.symbol.name + (fn.parameters.length ? '|' + fn.parameters
                        ...
                        .map(function (p) { return (p.name.text + " %" + p.name.text); }).join('|') : '');
                }
            }
            ...
            function computeVtableInfo(info) {
                ...
                for (var _b = 0, _c = info.methods; _b < _c.length; _b++) {
                    ...
                    if (prev) {
                        ...
                        pxtc.assert(pinf.virtualParent == pinf);
