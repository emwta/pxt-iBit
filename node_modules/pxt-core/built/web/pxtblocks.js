...
(function (pxt) {
    ...
    (function (blocks) {
        ...
        function mkPlaceholderBlock(e, type) {
            ...
            try {
                ...
                if (pxt.appTarget.compile) {
                    ...
                    upgrades.filter(function (up) { return up.type == "blockValue"; })
                        ...
                        var type = m[0];
                        var name = m[1];
                        pxt.Util.toArray(doc_1.querySelectorAll("block[type=" + type + "]>value[name=" + name + "]"))
                            .forEach(function (blockNode) {
        ...
        function createToolbox(blockInfo, toolbox, showCategories, filters) {
            ...
            if (tb && showCategories !== CategoryMode.None) {
                ...
                if (!config.textBlocks) {
                    ...
                    if (cat) {
                        var blockElements = cat.querySelectorAll("block");
                ...
                if (!config.listsBlocks) {
                    ...
                    showAdvanced = true;
                    var cat = categoryElement(tb, "Lists");
                    if (cat) {
                        var blockElements = cat.querySelectorAll("block");
                        for (var i = 0; i < blockElements.length; i++) {
                            var b = blockElements.item(i);
            ...
            if (filters) {
                ...
                if (showCategories !== CategoryMode.None) {
                    ...
                    var categories = tb.querySelectorAll("category:not([nameid=\"more\"]):not([nameid=\"advanced\"])");
                    for (var ci = 0; ci < categories.length; ++ci) {
                        ...
                        var blocks_8 = cat.querySelectorAll("block");
                        ...
                        switch (categoryState) {
                            case FilterState.Disabled:
                                if (!hasVisibleChildren) {
                                    ...
                                    var subcategories = cat.querySelectorAll("category");
                    ...
                    var blocks_9 = tb.querySelectorAll("block");
                    filterBlocks(blocks_9);
                }
                if (showCategories !== CategoryMode.None) {
                    // Go through all categories, hide the ones that have no blocks inside
                    var categories = tb.querySelectorAll("category:not([nameid=\"advanced\"])");
                    for (var ci = 0; ci < categories.length; ++ci) {
                        var cat = categories.item(ci);
                        var blockCount = cat.querySelectorAll("block");
                        ...
                        blocks.forEach(function (info) {
                            if (tb) {
                                ...
                                if (!block) {
                                    ...
                                    var existing = tb.querySelector("block[type=\"" + type + "\"]");
                                    if (existing) {
